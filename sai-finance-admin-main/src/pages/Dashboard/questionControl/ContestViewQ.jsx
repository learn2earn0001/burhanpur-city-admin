...
function ContestViewQ() {
  ...
  useEffect(() => {
    axios
    .get(`getContestQuestionsByContestId/${id}`)
    .then((res) => {
      if (res.data) {
  ...
  const handleDelete = () => {
    axios
      .delete(`deleteContestQuestion/${newID}`)
      .then((res) => {
        if (res.data) {
          console.log(res.data);
          toast({
          ...
          return (
            <>
              <Menu>
                ...
                <MenuList>
                  <Link to={`/dash/editcontest-question/${original._id}&${id}`}>
                    <MenuItem>
                      {" "}

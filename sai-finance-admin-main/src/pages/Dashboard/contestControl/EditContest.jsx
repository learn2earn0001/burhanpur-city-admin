...
const EditContest = () => {
        ...
        axios
            ...
            .then((res) => {
                if (res.data) {
                    ...
                    if (predata) {
                        const { _id, __v, createdOn, winersPrize, startTime, endTime, ...filteredPredata } = predata;
                        ...
                        setFormData({
                            ...filteredPredata,
                            winersPrize: winersPrize.join(', ')
                        });

                        setTimeDate({
                            startTimeHrs: startDateTime.toTimeString().slice(0, 5),
                            endTimeHrs: endDateTime.toTimeString().slice(0, 5),
    ...
    const handleSubmit = (e) => {
        ...
        axios.put(`updateContest/${id}`, processedFormData)
            .then((res) => {
                if (res.data) {
                    toast({
                        title: "Success! Contest Updated Successfully",
                        status: "success",
                        duration: 4000,
                        isClosable: true,
                        position: "top"
            ...
            <form onSubmit={handleSubmit}>
                <div className="bg-white shadow rounded-lg">
                    <div className="px-4 py-5 sm:px-6">
                        <h3 className="text-xl font-bold text-purple">Edit Contest ( <span className='text-bgBlue'>{product.contestName}</span>)</h3>
                    </div>
                    <div className="px-4 py-5 sm:p-6 space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm text-start font-medium text-gray-700">Winners Prize</label>
                                <input
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                    name="winersPrize"
